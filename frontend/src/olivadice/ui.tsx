...
export function ConfirmProvider({ children }: { children: React.ReactNode }) {
  ...
  return <ConfirmContext.Provider value={confirm}>{children}{request && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/50 p-4" onMouseDown={event => { if (event.target === event.currentTarget) close(false); }}>
    <div role="dialog" aria-modal="true" aria-labelledby="confirm-title" aria-describedby="confirm-message" className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
      <h2 id="confirm-title" className="text-lg font-semibold text-slate-900">{request.title || '确认操作'}</h2>
      <p id="confirm-message" className="mt-3 whitespace-pre-line text-sm leading-6 text-slate-600">{request.message}</p>
