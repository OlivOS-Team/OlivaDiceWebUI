...
function SettingRow({ item, save, reset }: { item: Setting; save: (key: string, value: number) => Promise<void>; reset: (key: string) => Promise<void> }) {
  ...
  return <div className="flex flex-col gap-3 border-b border-slate-100 py-4 last:border-0 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="flex items-center gap-2"><Button size="sm" variant="ghost" title={item.custom ? `删除 ${item.label}` : `恢复 ${item.label} 的默认值`} disabled={busy} onClick={() => void (async () => { if (await confirm(item.custom ? `删除扩展配置「${item.label}」？` : `恢复「${item.label}」的默认值？`, { confirmLabel: item.custom ? '删除' : '恢复', destructive: item.custom })) await reset(item.key); })()}><RotateCcw className="h-3.5 w-3.5" /><span className="sr-only">{item.custom ? '删除' : '恢复'} {item.label}</span></Button>
...
export function SettingsPage({ token, bot, notify }: Props) {
  ...
  const importFile = async (file?: File) => { if (!file) return; try { const data = await readJsonFile(file); if (await confirm(`将导入 ${Object.keys(data).length} 项配置到当前账号，继续吗？`, { confirmLabel: '导入' })) await apply('import', data); } catch (cause) { notify((cause as Error).message, true); } if (inputRef.current) inputRef.current.value = ''; };
  const confirmApply = async (message: string, action: string, destructive = false) => { if (await confirm(message, { confirmLabel: action === 'reload' ? '重新加载' : '恢复', destructive })) await apply(action); };
  ...
  return <><SectionTitle eyebrow="CORE CONFIG" title="配置列表" description="修改运行中的配置；单项保存立即写入青果骰配置文件。" />
    <div className="mb-5 flex flex-wrap gap-2"><input ref={inputRef} type="file" accept=".json,application/json" className="hidden" onChange={event => void importFile(event.target.files?.[0])} /><Button size="sm" variant="outline" disabled={busy} onClick={() => inputRef.current?.click()}><Upload className="mr-2 h-4 w-4" />导入配置</Button><Button size="sm" variant="outline" disabled={busy} onClick={exportFile}><Download className="mr-2 h-4 w-4" />导出配置</Button><Button size="sm" variant="outline" disabled={busy} onClick={() => void confirmApply('从磁盘重新加载当前账号配置？未保存的内存修改将被覆盖。', 'reload')}><RotateCcw className="mr-2 h-4 w-4" />从磁盘刷新</Button><Button size="sm" variant="outline" className="text-rose-600" disabled={busy} onClick={() => void confirmApply('确定将当前账号配置恢复默认值？骰主列表会保留。', 'reset', true)}>恢复全部默认值</Button></div>
