...
export function RepliesPage({ token, bot, notify, onDirtyChange }: Props) {
  ...
  const goToPage = (nextPage: number) => {
    if (nextPage === currentPage || !discard()) return;
    setPage(nextPage);
    setCreating(false);
  ...
  return <><SectionTitle eyebrow="CUSTOM REPLIES" title="回复词列表" description="按键名或内容定位回复；覆盖值与恢复模块按账号保存。" />
    ...
    {loading ? <Busy /> : <div className="grid min-h-[600px] gap-4 lg:grid-cols-[320px_minmax(0,1fr)]"><Card className="flex min-h-0 flex-col overflow-hidden border-slate-200 shadow-sm"><div className="space-y-3 border-b p-4"><SearchField value={query} onChange={value => updateFilter(value, modifiedOnly)} placeholder="搜索键名或回复内容" /><div className="flex items-center justify-between text-xs text-slate-500"><span>{filtered.length} / {items.length} 条回复</span><label className="flex cursor-pointer items-center gap-1.5"><input type="checkbox" checked={modifiedOnly} onChange={event => updateFilter(query, event.target.checked)} />仅看已修改</label></div></div><div ref={listRef} className="max-h-[620px] flex-1 overflow-y-auto p-2">{pageItems.length ? pageItems.map(item => <button key={item.key} onClick={() => choose(item.key)} className={`mb-1 w-full rounded-lg px-3 py-3 text-left transition-colors ${selected === item.key && !creating ? 'bg-brand-50 text-brand-800 ring-1 ring-brand-200' : 'hover:bg-slate-50'}`}><div className="flex items-center justify-between gap-2"><span className="truncate text-sm font-medium">{item.key}</span>{item.modified && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-brand-500" />}</div><p className="mt-1 truncate text-xs text-slate-500">{item.note || item.value || '空回复'}</p></button>) : <Empty title="没有匹配的回复" />}</div><div className="flex flex-wrap items-center justify-between gap-2 border-t px-3 py-2 text-xs text-slate-500"><span>{filtered.length ? `${(currentPage - 1) * PAGE_SIZE + 1}–${Math.min(currentPage * PAGE_SIZE, filtered.length)}` : '0'} / {filtered.length}</span><div className="flex items-center gap-2"><Button size="sm" variant="outline" disabled={currentPage <= 1} onClick={() => goToPage(currentPage - 1)}>上一页</Button><select aria-label="回复词页码" value={currentPage} onChange={event => goToPage(Number(event.target.value))} className="h-8 rounded-md border bg-white px-2 text-xs">{Array.from({ length: pageCount }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1} / {pageCount}</option>)}</select><Button size="sm" variant="outline" disabled={currentPage >= pageCount} onClick={() => goToPage(currentPage + 1)}>下一页</Button></div></div></Card>
