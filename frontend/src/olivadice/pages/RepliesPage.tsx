...
export function RepliesPage({ token, bot, notify, onDirtyChange }: Props) {
  ...
  const [unmodifiedOnly, setUnmodifiedOnly] = React.useState(false);
  ...
  const filtered = items.filter(item => (!modifiedOnly || item.modified) && (!unmodifiedOnly || !item.modified) && `${item.key} ${item.note} ${item.value}`.toLowerCase().includes(query.toLowerCase()));
  const pageCount = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const currentPage = Math.min(page, pageCount);
  const pageItems = filtered.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);
  const hasDraft = creating ? Boolean(newKey || draft) : Boolean(active && draft !== active.value);
  const updateFilter = (nextQuery: string, nextModifiedOnly: boolean, nextUnmodifiedOnly: boolean) => {
    const next = items.filter(item => (!nextModifiedOnly || item.modified) && (!nextUnmodifiedOnly || !item.modified) && `${item.key} ${item.note} ${item.value}`.toLowerCase().includes(nextQuery.toLowerCase()));
    if (!creating && !hasDraft && !next.some(item => item.key === selected)) setSelected(next[0]?.key || '');
    setQuery(nextQuery);
    setModifiedOnly(nextModifiedOnly);
    setUnmodifiedOnly(nextUnmodifiedOnly);
    setPage(1);
  ...
  const save = async () => {
    if (creating) { if (!newKey.trim()) { notify('请输入回复键', true); return; } try { const key = newKey.trim(); const updated = await manage('add', undefined, key, draft); setQuery(''); setModifiedOnly(false); setUnmodifiedOnly(false); setPage(Math.floor(Math.max(0, (updated || items).findIndex(item => item.key === key)) / PAGE_SIZE) + 1); setCreating(false); setSelected(key); } catch { /* shown by manage */ } return; }
    if (!active) return;
    setBusy(true);
  ...
  return <><SectionTitle eyebrow="CUSTOM REPLIES" title="回复词列表" description="按键名或内容定位回复；覆盖值与恢复模块按账号保存。" />
    ...
    {loading ? <Busy /> : <div className="grid min-h-[600px] gap-4 lg:grid-cols-[320px_minmax(0,1fr)]"><Card className="flex min-h-0 flex-col overflow-hidden border-slate-200 shadow-sm"><div className="space-y-3 border-b p-4"><SearchField value={query} onChange={value => updateFilter(value, modifiedOnly, unmodifiedOnly)} placeholder="搜索键名或回复内容" /><div className="flex items-center justify-between text-xs text-slate-500"><span>{filtered.length} / {items.length} 条回复</span><span className="flex items-center gap-3"><label className="flex cursor-pointer items-center gap-1.5"><input type="checkbox" checked={modifiedOnly} onChange={event => updateFilter(query, event.target.checked, false)} />仅看已修改</label><label className="flex cursor-pointer items-center gap-1.5"><input type="checkbox" checked={unmodifiedOnly} onChange={event => updateFilter(query, false, event.target.checked)} />仅看未修改</label></span></div></div><div ref={listRef} className="max-h-[620px] flex-1 overflow-y-auto p-2">{pageItems.length ? pageItems.map(item => <button key={item.key} onClick={() => void choose(item.key)} className={`mb-1 w-full rounded-lg px-3 py-3 text-left transition-colors ${selected === item.key && !creating ? 'bg-brand-50 text-brand-800 ring-1 ring-brand-200' : 'hover:bg-slate-50'}`}><div className="flex items-center justify-between gap-2"><span className="truncate text-sm font-medium">{item.key}</span>{item.modified && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-brand-500" />}</div><p className="mt-1 truncate text-xs text-slate-500">{item.note || item.value || '空回复'}</p></button>) : <Empty title="没有匹配的回复" />}</div><div className="flex flex-wrap items-center justify-between gap-2 border-t px-3 py-2 text-xs text-slate-500"><span>{filtered.length ? `${(currentPage - 1) * PAGE_SIZE + 1}–${Math.min(currentPage * PAGE_SIZE, filtered.length)}` : '0'} / {filtered.length}</span><div className="flex items-center gap-2"><Button size="sm" variant="outline" disabled={currentPage <= 1} onClick={() => void goToPage(currentPage - 1)}>上一页</Button><Select ariaLabel="回复词页码" size="sm" className="w-24" placement="top" value={String(currentPage)} onChange={value => void goToPage(Number(value))} options={Array.from({ length: pageCount }, (_, index) => ({ value: String(index + 1), label: `${index + 1} / ${pageCount}` }))} /><Button size="sm" variant="outline" disabled={currentPage >= pageCount} onClick={() => void goToPage(currentPage + 1)}>下一页</Button></div></div></Card>
