...
export function RepliesPage({ token, bot, notify, onDirtyChange }: Props) {
  ...
  const discard = () => !hasDraft || window.confirm('当前回复尚未保存，确定放弃修改吗？');
  const choose = (key: string) => { if (!discard()) return; setCreating(false); setSelected(key); };
  const create = () => { if (!discard()) return; setCreating(true); setSelected(''); setNewKey(''); setDraft(''); };
  const manage = async (action: string, data?: unknown, key?: string, value?: string) => {
    setBusy(true);
    try { await api('/api/replies/manage', token, { bot, action, data, key, value }); await load(); notify('回复词已更新'); }
    catch (cause) { notify((cause as Error).message, true); throw cause; }
    finally { setBusy(false); }
  };
  const save = async () => {
    if (creating) { if (!newKey.trim()) { notify('请输入回复键', true); return; } try { await manage('add', undefined, newKey.trim(), draft); setCreating(false); setSelected(newKey.trim()); } catch { /* shown by manage */ } return; }
    if (!active) return;
    setBusy(true);
  ...
  const importJson = async (file?: File) => { if (!file) return; try { const data = await readJsonFile(file); if (window.confirm(`将导入 ${Object.keys(data).length} 条回复；文件中的同名回复会覆盖当前值，继续吗？`)) await manage('import', data); } catch (cause) { notify((cause as Error).message, true); } if (inputRef.current) inputRef.current.value = ''; };
  ...
  return <><SectionTitle eyebrow="CUSTOM REPLIES" title="回复词列表" description="按键名或内容定位回复；覆盖值与恢复模块按账号保存。" />
    <div className="mb-5 flex flex-wrap gap-2"><input ref={inputRef} type="file" accept=".json,application/json" className="hidden" onChange={event => void importJson(event.target.files?.[0])} /><Button size="sm" variant="outline" onClick={() => inputRef.current?.click()} disabled={busy}><Upload className="mr-2 h-4 w-4" />导入回复</Button><Button size="sm" variant="outline" onClick={() => void exportJson()} disabled={busy}><Download className="mr-2 h-4 w-4" />导出 JSON</Button><Button size="sm" variant="outline" onClick={() => { if (window.confirm('从磁盘重新加载回复词？未保存的修改将被覆盖。')) void manage('reload'); }} disabled={busy}><RotateCcw className="mr-2 h-4 w-4" />从磁盘刷新</Button><Button size="sm" variant="outline" onClick={() => setModulesOpen(current => !current)}><Settings2 className="mr-2 h-4 w-4" />恢复模块</Button><Button size="sm" variant="outline" className="text-rose-600" onClick={() => { if (window.confirm('恢复当前账号已加载模块的默认回复？已有自定义键会保留。')) void manage('reset'); }} disabled={busy}>恢复模块默认</Button><Button size="sm" onClick={create}><Plus className="mr-2 h-4 w-4" />新增回复</Button></div>
    {modulesOpen && <Card className="mb-5 border-indigo-200 bg-indigo-50/40"><CardContent className="p-5"><div className="font-medium">恢复模块列表</div><p className="mt-1 text-xs text-slate-500">每行一个插件命名空间；恢复默认时，从已加载模块读取默认回复。清空后保存会恢复标准列表。</p><Textarea className="mt-3 min-h-28 font-mono text-sm" value={modulesDraft} onChange={event => setModulesDraft(event.target.value)} aria-label="恢复模块列表" /><div className="mt-3 flex justify-end"><Button size="sm" onClick={() => void saveModules()} disabled={busy || modulesDraft === modules.join('\n')}>保存模块列表</Button></div></CardContent></Card>}
    {loading ? <Busy /> : <div className="grid min-h-[600px] gap-4 lg:grid-cols-[320px_minmax(0,1fr)]"><Card className="flex min-h-0 flex-col overflow-hidden border-slate-200 shadow-sm"><div className="space-y-3 border-b p-4"><SearchField value={query} onChange={value => updateFilter(value, modifiedOnly)} placeholder="搜索键名或回复内容" /><div className="flex items-center justify-between text-xs text-slate-500"><span>{filtered.length} / {items.length} 条回复</span><label className="flex cursor-pointer items-center gap-1.5"><input type="checkbox" checked={modifiedOnly} onChange={event => updateFilter(query, event.target.checked)} />仅看已修改</label></div></div><div className="max-h-[620px] flex-1 overflow-y-auto p-2">{filtered.length ? filtered.slice(0, 200).map(item => <button key={item.key} onClick={() => choose(item.key)} className={`mb-1 w-full rounded-lg px-3 py-3 text-left transition-colors ${selected === item.key && !creating ? 'bg-indigo-50 text-indigo-800 ring-1 ring-indigo-200' : 'hover:bg-slate-50'}`}><div className="flex items-center justify-between gap-2"><span className="truncate text-sm font-medium">{item.key}</span>{item.modified && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-indigo-500" />}</div><p className="mt-1 truncate text-xs text-slate-500">{item.note || item.value || '空回复'}</p></button>) : <Empty title="没有匹配的回复" />}{filtered.length > 200 && <div className="p-3 text-center text-xs text-slate-500">只显示前 200 条，请缩小搜索范围</div>}</div></Card>
      <Card className="border-slate-200 shadow-sm"><CardContent className="p-5 md:p-7">{creating || active ? <><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="mb-2 flex items-center gap-2 text-xs font-medium text-indigo-600"><Sparkles className="h-4 w-4" />{creating ? '新增自定义回复' : active?.default === null ? '自定义回复' : '当前词条'}</div>{creating ? <Input className="mt-2 max-w-sm font-mono" placeholder="新回复键，例如 strMyReply" value={newKey} onChange={event => setNewKey(event.target.value)} maxLength={200} /> : <h3 className="break-all font-mono text-xl font-semibold text-slate-900">{active?.key}</h3>}<p className="mt-2 whitespace-pre-line text-xs text-slate-500">{active?.note || '保留原有模板变量写法'}</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">{draft.length} / 20000 字</span></div><div className="mt-6"><label htmlFor="reply-content" className="mb-2 block text-sm font-medium">回复内容</label><Textarea id="reply-content" className="min-h-[320px] resize-y bg-white font-mono text-sm leading-6" value={draft} maxLength={20000} onChange={event => setDraft(event.target.value)} /></div><div className="mt-4 flex flex-wrap justify-between gap-2"><div>{active?.default === null && !creating && <Button variant="outline" className="text-rose-600" disabled={busy} onClick={() => { if (window.confirm(`删除自定义回复「${active.key}」？`)) void manage('delete', undefined, active.key); }}><Trash2 className="mr-2 h-4 w-4" />删除</Button>}</div><div className="flex gap-2">{active?.default !== null && active?.modified && !creating && <Button variant="outline" onClick={() => void resetOne()} disabled={busy}><RotateCcw className="mr-2 h-4 w-4" />恢复默认</Button>}<Button onClick={() => void save()} disabled={busy || (!creating && draft === active?.value)}><Save className="mr-2 h-4 w-4" />{creating ? '添加回复' : '保存回复'}</Button></div></div></> : <Empty title="选择左侧回复词或新增" />}</CardContent></Card></div>}
  </>;
}
