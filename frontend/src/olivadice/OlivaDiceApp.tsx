...
export function OlivaDiceApp() {
  ...
  const connect = React.useCallback(async (key: string) => { setLoading(true); try { const result = await api<{ accounts: Account[]; version: string }>('/api/accounts', key); setAccounts(result.accounts); setVersion(result.version); const stored = standalone ? sessionStorage.getItem('olivadice-bot') : null; const next = stored && result.accounts.some(item => item.hash === stored) ? stored : result.accounts.find(item => item.hash !== 'unity')?.hash || 'unity'; setBot(current => result.accounts.some(item => item.hash === current) ? current : next); if (standalone) { sessionStorage.setItem('olivadice-bot', next); sessionStorage.setItem('olivadice-token', key); setToken(key); } setConnected(true); notify(''); } catch (cause) { if (standalone) sessionStorage.removeItem('olivadice-token'); setConnected(false); notify((cause as Error).message, true); } finally { setLoading(false); } }, [notify]);
  ...
  const logout = async () => { if (dirty.current) { if (!(await confirm('当前修改尚未保存，确定退出吗？', { confirmLabel: '放弃修改', destructive: true }))) return; dirty.current = false; } sessionStorage.removeItem('olivadice-token'); setToken(''); setConnected(false); setAccounts([]); setVersion(''); setInputToken(''); notify(''); };
  ...
  return <div className="min-h-screen bg-[#f5faff] text-slate-900">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-slate-200 bg-white transition-transform lg:translate-x-0 ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}><div className="flex h-16 items-center gap-3 border-b px-5"><img src={olivaLogo} alt="青果骰 OlivaDice" className="h-10 w-10 shrink-0 object-contain" /><div className="min-w-0"><div className="text-base font-bold tracking-tight text-brand-700">青果骰</div><div className="text-[10px] font-medium uppercase tracking-wider text-slate-400">OlivaDice WebUI</div></div><button className="ml-auto lg:hidden" aria-label="关闭菜单" onClick={() => setMenuOpen(false)}><X className="h-5 w-5" /></button></div><nav className="flex-1 space-y-6 overflow-y-auto px-3 py-6">{nav.map(group => <div key={group.heading}><div className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">{group.heading}</div><div className="space-y-1">{group.items.map(({ id, icon: Icon }) => <button key={id} onClick={() => void navigate(id)} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors ${view === id ? 'bg-brand-50 text-brand-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}><Icon className="h-[18px] w-[18px]" /><span>{labels[id]}</span>{view === id && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-brand-500" />}</button>)}</div></div>)}</nav><div className="border-t p-4"><div className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2.5 text-xs text-slate-600"><span className={`h-2 w-2 rounded-full ${connected ? 'bg-emerald-500' : 'bg-slate-300'}`} />{connected ? '管理服务已连接' : '等待连接'}<span className="ml-auto text-slate-400">{version && `v${version}`}</span></div></div></aside>
    <div className="min-w-0 lg:pl-64"><header className="sticky top-0 z-20 flex h-16 items-center justify-between gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur md:px-8"><div className="flex min-w-0 items-center gap-3"><Button size="icon" variant="ghost" className="lg:hidden" onClick={() => setMenuOpen(true)}><Menu className="h-5 w-5" /></Button><div className="truncate text-xs text-slate-500">青果骰 <span className="mx-1 text-slate-300">/</span> <span className="font-medium text-slate-800">{labels[view]}</span></div></div><div className="flex items-center gap-2">{connected && <><div className="relative hidden sm:block"><select aria-label="当前账号" value={bot} onChange={event => void selectBot(event.target.value)} className="h-9 max-w-56 appearance-none truncate rounded-lg border bg-white py-1 pl-3 pr-8 text-xs font-medium text-slate-700 outline-none focus:ring-2 focus:ring-brand-300">{accounts.map(account => <option key={account.hash} value={account.hash}>{account.label}</option>)}</select><ChevronDown className="pointer-events-none absolute right-2 top-2.5 h-4 w-4 text-slate-400" /></div><Button size="sm" variant="ghost" onClick={() => void connect(token)} title="刷新账号列表"><RotateCcw className="h-4 w-4" /><span className="ml-2 hidden md:inline">刷新账号</span></Button>{standalone && <Button size="sm" variant="ghost" onClick={() => void logout()} title="退出管理"><LogOut className="h-4 w-4" /><span className="ml-2 hidden md:inline">退出</span></Button>}</>}</div></header>
      <main className="mx-auto max-w-[1440px] px-4 py-6 md:px-8 md:py-8"><div className="mb-6 flex flex-wrap items-end justify-between gap-3"><div><div className="mb-1 text-[11px] font-bold uppercase tracking-[0.18em] text-brand-600">管理面板</div><h1 className="text-2xl font-bold tracking-tight md:text-3xl">{labels[view]}</h1><p className="mt-1 text-sm text-slate-500">{view === 'backup' || view === 'server' ? '当前范围：全局配置' : `当前范围：${current?.label || '全局配置'}`}</p></div>{connected && <select aria-label="移动端账号选择" value={bot} onChange={event => void selectBot(event.target.value)} className="h-9 max-w-full rounded-lg border bg-white px-3 text-xs text-slate-700 sm:hidden">{accounts.map(account => <option key={account.hash} value={account.hash}>{account.label}</option>)}</select>}</div>
      <Notice message={notice.message} error={notice.error} onClose={() => notify('')} />
