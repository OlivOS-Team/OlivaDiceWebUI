...
export function OlivaDiceApp() {
  ...
  const page = <main className="mx-auto max-w-[1440px] px-4 py-6 md:px-8 md:py-8"><div className="mb-6 flex flex-wrap items-end justify-between gap-3"><div><div className="mb-1 text-[11px] font-bold uppercase tracking-[0.18em] text-brand-600">管理面板</div><h1 className="text-2xl font-bold tracking-tight md:text-3xl">{labels[view]}</h1><p className="mt-1 text-sm text-muted-foreground">{view === 'backup' || view === 'server' ? '当前范围：全局设置' : `当前范围：${current?.label || '全局设置'}`}</p></div>{connected && accountSelect(true)}</div>
    <Notice message={notice.message} error={notice.error} onClose={() => notify('')} />
