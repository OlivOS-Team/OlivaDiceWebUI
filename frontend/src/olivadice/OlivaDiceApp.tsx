...
export function OlivaDiceApp() {
  ...
  const selectBot = (next: string) => { if (next !== bot && dirty.current) { if (!window.confirm('当前修改尚未保存，确定切换账号吗？')) return; dirty.current = false; } setBot(next); if (standalone) sessionStorage.setItem('olivadice-bot', next); notify(''); };
