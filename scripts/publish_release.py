...
ARCHIVE_SPECS = {
    'OlivaDiceWebUI/app.json': ('OlivaDiceWebUI', 'OlivaDice WebUI（官方接入版）'),
    'OlivaDiceWebUIStandalone/app.json': (
...
def identify_archive(source):
    with ZipFile(source) as archive:
        matches = [path for path in ARCHIVE_SPECS if path in archive.namelist()]
    if len(matches) != 1:
        raise ValueError('Plugin archive must contain exactly one known app.json')
    manifest_path = matches[0]
    base_name, label = ARCHIVE_SPECS[manifest_path]
    return manifest_path, base_name, label
...
def stamp_archive(source, version):
    ...
    manifest_path, base_name, _ = identify_archive(source)
    # GitHub normalizes parentheses in uploaded asset filenames to periods.
    target = source.with_name('{}-{}.{}.zip'.format(base_name, *match.groups()))
...
def main():
    ...
    if len(sources) != len(ARCHIVE_SPECS):
        raise SystemExit('Usage: publish_release.py <official.zip> <standalone.zip>')
    for source in sources:
        if not source.is_file():
            raise SystemExit('Package not found: {}'.format(source))
    identified = [(source, identify_archive(source)) for source in sources]
    manifests = {spec[0] for _, spec in identified}
    if manifests != set(ARCHIVE_SPECS):
